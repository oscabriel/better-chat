...
import { getCurrentUsageSummary } from "@/server/utils/usage-service";
import { requireUserId, UnauthorizedError } from "./guard";
...
export async function rateLimitMiddleware(c: Context, next: Next) {
	try {
		...
		// Attach usage info to context for use by route handlers
		c.set("rateLimitInfo", {
			dailyUsed: usage.daily.used,
			monthlyUsed: usage.monthly.used,
		});
